// script.js — Global Conflict Tracker
...
fetch(CONFLICTS_URL)
  ...
  .then(data => {
    ...
    sourceLabel.textContent = "Online JSON";
...
// Создание списка стран для фильтра
...
function setupYearFilter() {
  const years = conflictsData.map(c => c.year).filter(y => Number.isFinite(y));
  const minYear = Math.min(...years);
  const maxYear = Math.max(...years);
  yearFilter.min = minYear;
  yearFilter.max = maxYear;
  yearFilter.value = 0; // 0 = All
  yearLabel.textContent = "All";
}

// Фильтрация и рендер карты
function renderMap() {
  ...
  filtered.forEach(c => {
    ...
    if (c.actor1 && c.actor2) {
      ...
      ], { color: 'yellow', weight: 2, opacity: 0.9 });
      linesLayer.addLayer(line);
    }

    heatPoints.push([c.lat, c.lon, 0.8]);
  });
