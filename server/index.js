...
app.get('/api/update-conflicts', requireAdmin, async (_req, res) => {
  try {
    ...
    res.json({
      ...
      rawTextLength: detection.rawTextLength,
      extractedCount: detection.extractedCount,
      rawPreview: detection.rawPreview,
      suggestedCount: suggested.length,
      message: suggested.length ? 'AI suggestions refreshed.' : 'AI returned no valid conflict records.'
    })
  } catch (error) {
    ...
    res.status(500).json({
      ...
      rawPreview: '',
      suggestedCount: suggested.length,
      message: error instanceof Error ? error.message : 'AI refresh failed.'
    })
  }
})
...
app.post('/api/apply-conflicts', requireAdmin, async (_req, res) => {
  try {
    ...
    res.status(500).json({
      mode: 'applied',
      message: error instanceof Error ? error.message : 'Failed to apply suggested conflicts.'
    })
  }
})
